...
export const ImageUploader: React.FC<ImageUploaderProps> = ({ onImageSelected, initialMode = 'PHOTO' }) => {
  ...
  // Video Streaming to Live Service
  useEffect(() => {
      let interval: any;
      if (mode === 'VOICE' && !liveError && liveServiceRef.current) {
          interval = setInterval(() => {
              const frame = captureFrameData(480);
              if (frame) {
                  liveServiceRef.current?.sendVideoFrame(frame.data);
              }
          }, 800); // 1.25 fps video stream
      }
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
                    {liveHistory.length === 0 ? (
                        <div className="text-center text-slate-500 py-10 flex flex-col items-center">
                            ...
                            <p className="text-sm">Warte auf Erkennung...</p>
                            <p className="text-xs mt-1 opacity-50">Sprich mit der KI über Gegenstände.</p>
                        </div>
                    ) : (
